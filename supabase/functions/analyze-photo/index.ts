...
IMPORTANT — avoid hallucinations: Only report items that are clearly visible in the photograph. Do NOT add items merely because they are typical or expected for this room type — if you cannot see it in the image, do not include it. For example, do not list "Oven / Cooktop", "Sink", or "Rangehood" for a kitchen photo that only shows a wall and floor. If the photo shows a close-up of a single item (e.g. a door handle, a crack in a wall, a tap), report only that item and closely related components — do not pad the response with unrelated room-wide items. Prefer returning fewer items with high confidence over many uncertain guesses. Always include a confidence score between 0 and 1 for every item; use values below 0.7 for anything you are not sure about.
...
ROOM-TYPE CONTEXT: You will be given the current room_type (e.g. "kitchen", "bathroom", "lounge", "hallway", "bedroom"). The room type is provided for context only — it should NOT cause you to infer or add items that are not visible. Flag any item that is atypical for this room type with lowConfidence: true regardless of visual confidence — for example, a fireplace in a kitchen, an oven in a bathroom, or lounge furniture in a hallway. Also set lowConfidence: true whenever confidence is below 0.7 or you are uncertain the item is really present. Every returned item MUST include both a numeric "confidence" (0..1) AND a boolean "lowConfidence" field.`;
