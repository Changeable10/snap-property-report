...
interface Photo { id: string; room_id: string; photo_url: string; captured_at: string }
...
function refNumber(inspectionId: string) {
...
function ReportPage() {
  ...
  useEffect(() => {
    if (!inspection || !property || !rooms || !items || !photos) return;
    let cancelled = false;
    ...
    (async () => {
      ...
      const addressLine = `${property.address}, ${property.suburb}, ${property.city} ${property.postcode}`;
      const fname = `${slugify(property.address)}-${inspection.inspection_type}-${inspection.inspection_date}.pdf`;
      setFilename(fname);
      ...
      const photosByRoom = new Map<string, Photo[]>();
      ...
      const drawSigBlock = (label: string, startY: number) => {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(11);
        doc.setTextColor(15, 110, 86);
        ...
        const rows = ["Name", "Signature", "Date"];
        let yy = startY + 8;
        for (const r of rows) {
          doc.text(r, margin, yy);
          doc.line(margin + 25, yy + 1, pageW - margin, yy + 1);
          yy += 12;
        }
        return yy;
      };
      const afterLandlord = drawSigBlock("Landlord / Agent", y);
      drawSigBlock("Tenant", afterLandlord + 8);
  ...
  }, [inspection, property, rooms, items, photos]);
